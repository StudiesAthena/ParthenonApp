...
export const Reports: React.FC<ReportsProps> = ({ calendar }) => {
  ...
  const MetricCard = ({ title, value, unit, icon: Icon, colorClass, textColor }: any) => (
    <div className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-3xl border-2 border-slate-200 dark:border-slate-800 shadow-md flex items-center gap-5">
      <div className={`p-3.5 rounded-2xl ${colorClass} text-white shadow-lg`}>
        <Icon size={26} />
      </div>
      <div>
        <p className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">{title}</p>
        <p className={`text-xl md:text-2xl font-black ${textColor || 'text-slate-950 dark:text-white'}`}>
          {value} <span className="text-xs font-black text-slate-500">{unit}</span>
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border-2 border-slate-200 dark:border-slate-800 shadow-md">
          <h3 className="text-[11px] font-black text-slate-900 dark:text-slate-400 uppercase tracking-widest mb-8 flex items-center gap-3">
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={last7Days} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '1.5rem', border: '2px solid #e2e8f0', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)', fontWeight: 'bold'}} />
        ...
        <div className="bg-white dark:bg-slate-900 p-7 rounded-[2.5rem] border-2 border-slate-200 dark:border-slate-800 space-y-5 shadow-md">
          <h3 className="text-[11px] font-black text-slate-900 dark:text-slate-400 uppercase tracking-widest mb-4">Resumo Geral</h3>
          
          <div className="p-5 bg-slate-100 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            <p className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Acumulado do Ano</p>
            <p className="text-xl font-black text-athena-teal dark:text-white mt-1">{formatDuration(stats.year.total)} <span className="text-xs font-bold text-slate-500">/ {stats.year.days} dias</span></p>
          </div>

          <div className="p-5 bg-slate-100 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            <p className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Performance Mensal</p>
            <p className="text-xl font-black text-emerald-700 dark:text-emerald-400 mt-1">{formatDuration(stats.month.total)} <span className="text-xs font-bold text-slate-500">/ {stats.month.days} dias</span></p>
          </div>

          <div className="p-5 bg-slate-100 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            <p className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Ritmo da Semana</p>
            <p className="text-xl font-black text-[#FF7E67] dark:text-[#FF7E67] mt-1">{formatDuration(stats.week.total)} <span className="text-xs font-bold text-slate-500">/ {stats.week.days} dias</span></p>
          </div>
        </div>
      </div>
    </div>
